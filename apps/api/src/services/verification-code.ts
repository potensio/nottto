import { eq, and, isNull } from "drizzle-orm";
import { HTTPException } from "hono/http-exception";
import { db } from "../db";
import {
  users,
  verificationCodes,
  workspaces,
  workspaceMembers,
  projects,
  magicLinkTokens,
} from "@notto/shared/db";
import {
  hashToken,
  verifyTokenHash,
  getTokenExpiration,
  isTokenExpired,
  maskEmail,
  generateSecureToken,
} from "../utils/magic-link";
import { generateTokens } from "../utils/auth";
import { generateUniqueSlug } from "../utils/slug";
import { sendVerificationCodeEmail } from "./email";
import { checkMagicLinkLimit, recordMagicLinkRequest } from "./rate-limiter";
import type { User, AuthResponse } from "@notto/shared";

const WEB_URL = process.env.WEB_URL || "http://localhost:3000";

/**
 * Generates a 6-digit verification code
 */
function generateVerificationCode(): string {
  return Math.floor(100000 + Math.random() * 900000).toString();
}

export interface VerificationCodeRequestResult {
  message: string;
  email: string; // Masked email
}

export interface VerificationCodeVerifyResult extends AuthResponse {
  isNewUser: boolean;
  magicLinkUrl: string; // URL to redirect user for web app login
}

/**
 * Requests a verification code to be sent to the provided email.
 * Distinguishes between login (existing users) and register (new users).
 */
export async function requestVerificationCode(
  email: string,
  isRegister: boolean = false,
  name?: string,
): Promise<VerificationCodeRequestResult> {
  const normalizedEmail = email.toLowerCase().trim();

  // Check rate limit (reuse magic link rate limiter)
  const rateLimit = await checkMagicLinkLimit(normalizedEmail);
  if (!rateLimit.allowed) {
    throw new HTTPException(429, {
      message: "Too many requests. Please try again later.",
    });
  }

  // Check if user exists
  const [existingUser] = await db
    .select()
    .from(users)
    .where(eq(users.email, normalizedEmail))
    .limit(1);

  // Handle login vs register distinction
  if (isRegister) {
    // Registration: user should NOT exist
    if (existingUser) {
      throw new HTTPException(409, {
        message:
          "An account with this email already exists. Please login instead.",
      });
    }
    // Validate name for registration
    if (!name || name.trim().length === 0) {
      throw new HTTPException(400, {
        message: "Full name is required for registration.",
      });
    }
  } else {
    // Login: user SHOULD exist
    if (!existingUser) {
      throw new HTTPException(404, {
        message: "No account found with this email. Please register first.",
      });
    }
  }

  // Generate 6-digit code
  const code = generateVerificationCode();
  const codeHash = hashToken(code);
  const expiresAt = getTokenExpiration(10); // 10 minutes

  // Store hashed code with registration data
  await db.insert(verificationCodes).values({
    email: normalizedEmail,
    codeHash,
    name: isRegister ? name?.trim() : null,
    isRegister,
    expiresAt,
  });

  // Record rate limit
  await recordMagicLinkRequest(normalizedEmail);

  // Send email with code
  const emailResult = await sendVerificationCodeEmail(normalizedEmail, code);
  if (!emailResult.success) {
    throw new HTTPException(500, {
      message: "Failed to send email. Please try again.",
    });
  }

  return {
    message: "Verification code sent! Check your email.",
    email: maskEmail(normalizedEmail),
  };
}

/**
 * Verifies a verification code and returns session tokens.
 * Creates user, workspace, and project if this is a new user (registration).
 */
export async function verifyVerificationCode(
  email: string,
  code: string,
): Promise<VerificationCodeVerifyResult> {
  const normalizedEmail = email.toLowerCase().trim();
  const codeHash = hashToken(code);

  // Find code by hash and email
  const [codeRecord] = await db
    .select()
    .from(verificationCodes)
    .where(
      and(
        eq(verificationCodes.email, normalizedEmail),
        eq(verificationCodes.codeHash, codeHash),
        isNull(verificationCodes.usedAt),
      ),
    )
    .limit(1);

  if (!codeRecord) {
    throw new HTTPException(401, {
      message: "Invalid or expired code. Please request a new one.",
    });
  }

  // Check expiration
  if (isTokenExpired(codeRecord.expiresAt)) {
    // Mark as used to prevent future attempts
    await db
      .update(verificationCodes)
      .set({ usedAt: new Date() })
      .where(eq(verificationCodes.id, codeRecord.id));

    throw new HTTPException(401, {
      message: "This code has expired. Please request a new one.",
    });
  }

  // Mark code as used
  await db
    .update(verificationCodes)
    .set({ usedAt: new Date() })
    .where(eq(verificationCodes.id, codeRecord.id));

  // Find or create user
  let [existingUser] = await db
    .select()
    .from(users)
    .where(eq(users.email, normalizedEmail))
    .limit(1);

  let isNewUser = false;

  if (!existingUser) {
    isNewUser = true;

    // Create new user with name from code record (for registration)
    const [newUser] = await db
      .insert(users)
      .values({
        email: normalizedEmail,
        name: codeRecord.name || null,
        passwordHash: null, // Passwordless user
      })
      .returning();

    existingUser = newUser;

    // Create default workspace
    const existingWorkspaces = await db
      .select({ slug: workspaces.slug })
      .from(workspaces);
    const existingSlugs = existingWorkspaces.map((w) => w.slug);
    const workspaceName = codeRecord.name || "Personal";
    const workspaceSlug = generateUniqueSlug(workspaceName, existingSlugs);

    const [newWorkspace] = await db
      .insert(workspaces)
      .values({
        name: workspaceName,
        slug: workspaceSlug,
        icon: "📁",
        ownerId: newUser.id,
      })
      .returning();

    // Add user as workspace owner
    await db.insert(workspaceMembers).values({
      workspaceId: newWorkspace.id,
      userId: newUser.id,
      role: "owner",
    });

    // Create default project
    const existingProjects = await db
      .select({ slug: projects.slug })
      .from(projects)
      .where(eq(projects.workspaceId, newWorkspace.id));
    const existingProjectSlugs = existingProjects.map((p) => p.slug);
    const projectSlug = generateUniqueSlug(
      "My First Project",
      existingProjectSlugs,
    );

    await db.insert(projects).values({
      workspaceId: newWorkspace.id,
      name: "My First Project",
      slug: projectSlug,
      description: "Your first project - start capturing feedback!",
    });
  }

  // Generate session tokens
  const tokens = await generateTokens({
    sub: existingUser.id,
    email: existingUser.email,
  });

  const user: User = {
    id: existingUser.id,
    email: existingUser.email,
    name: existingUser.name,
    profilePicture: existingUser.profilePicture,
    createdAt: existingUser.createdAt,
    updatedAt: existingUser.updatedAt,
  };

  // Generate a magic link token for web app login
  const magicToken = generateSecureToken();
  const magicTokenHash = hashToken(magicToken);
  const magicExpiresAt = getTokenExpiration(15); // 15 minutes

  // Store magic link token
  await db.insert(magicLinkTokens).values({
    email: normalizedEmail,
    tokenHash: magicTokenHash,
    name: existingUser.name,
    isRegister: false, // User already exists/created
    expiresAt: magicExpiresAt,
  });

  // Build magic link URL for web app
  const magicLinkUrl = `${WEB_URL}/auth/verify?token=${encodeURIComponent(
    magicToken,
  )}`;

  return { user, tokens, isNewUser, magicLinkUrl };
}
